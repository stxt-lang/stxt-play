...
in the browser, with its grammar next to it, and see errors as you type. It is a real editor in the
spirit of VS Code, and not a form or a viewer.
...
list and was dropped; it may come back as a secondary view, but it is not what the playground is
about.
...
that is not a grammar, or does not decode, is ignored, and the document of the link always opens.
...
- Schema discovery (STXT-DISCOVERY-SPEC) assumes a filesystem. The browser has no directory chain,
  ...
  same rule STXT-DISCOVERY-SPEC applies to two definitions at the same level.
