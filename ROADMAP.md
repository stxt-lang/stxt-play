...
- [x] Documentos y gramáticas de ejemplo precargados (del corpus de `../stxt-lang`) y botón de
      reset.
...
corpus de `../stxt-lang`: `email.stxt`, la plantilla `com.example.docs` y el esquema tutorial
...
La familia `stxt.play.*` queda reservada para la semilla (regla en la ficha de `stxt-lang`);
...
- [x] Enlace desde `stxt.dev`, vía `../stxt-cms` (2026-08-15): entrada *Playground* en la barra
      ...
      `../stxt-lang`, en y es; `node.vm` aprendió el `Url` externo y `navigation.vm` lo salta en
